...
export function PapeleraConfig() {
  ...
  const cargar = () => {
    setCargando(true);
    fetch(`${SUPABASE_URL}/rest/v1/predictamen?select=id,folio,posicion,expediente,estado,dictamen_sugerido,papelera_fecha,datos&en_papelera=eq.true&order=papelera_fecha.desc`, { headers })
      .then((r) => (r.ok ? r.json() : Promise.reject(new Error(`Supabase ${r.status}`))))
      .then(setFilas).catch((e) => setError(e.message + " (¿corriste el SQL de papelera?)")).finally(() => setCargando(false));
  };
  useEffect(cargar, []);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">{cargando ? "Cargando…" : `${filas.length} en la papelera`} · los pre-dictámenes enviados aquí no aparecen en el historial, pero se pueden recuperar.</p>
