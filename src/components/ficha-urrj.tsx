  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {/* Vista previa (solo lectura) — TODAS las respuestas capturadas, agrupadas por hito */}
        {preview === "juridico" && predJur && (
          <div className="mt-3 rounded-xl border border-border bg-card p-3 text-xs">
            <div className="mb-2 flex items-center justify-between">
              <p className="text-sm font-medium text-foreground">👁 Vista previa · Jurídico (solo lectura)</p>
              <button onClick={() => abrirProceso("juridico")} className="inline-flex items-center gap-1 rounded-md border border-input bg-white px-2.5 py-1 text-[11px] font-medium hover:bg-muted">
                ← Volver al proceso de dictaminación
              </button>
            </div>
            <div className="mb-2 grid grid-cols-2 gap-x-3 gap-y-0.5 text-muted-foreground sm:grid-cols-4">
              <div>Posición: <b className="text-foreground">{predJur.posicion || "—"}</b></div>
              <div>Exp.: <b className="text-foreground">{garantia.expediente || "—"}</b></div>
...
// cuestionario de URRJ (posición Actor), agrupado por hito,
// en modo solo lectura — para que Dirección/quien revise no
// tenga que reabrir el proceso de dictaminación para ver qué
// se llenó. Solo pinta los grupos que tengan al menos un dato.
// ============================================================
...
function VistaPreviaRespuestas({ datos }: { datos: any }) {
