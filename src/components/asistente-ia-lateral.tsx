...
interface CasoResultado { id: string; cliente_nombre: string; cliente_codigo: string; gar_id: string; expediente: string; unidad: string }
...
function ChatCasoEspecifico() {
  ...
  async function buscarCasos() {
    ...
    try {
      ...
      const campos = "id,cliente_nombre,cliente_codigo,gar_id,expediente,unidad";
      const filtro = `or=(cliente_nombre.ilike.${q},cliente_codigo.ilike.${q},gar_id.ilike.${q},expediente.ilike.${q})`;
      const r = await fetch(`${SUPABASE_URL}/rest/v1/caso_juridico?select=${campos}&${filtro}&limit=15`, {
  ...
  if (casoElegido) {
    return <ChatDeCaso caso={casoElegido} onSalir={() => { setCasoElegido(null); setResultados([]); setBusqueda(""); }} />;
  }

  return (
    <div className="flex h-full flex-col p-3">
      <p className="mb-2 text-xs text-neutral-500">Busca por nombre del cliente, código, garantía o expediente:</p>
      <div className="flex gap-2">
        <input
          value={busqueda}
          onChange={(e) => setBusqueda(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && buscarCasos()}
          placeholder="Ej: García, GAR-2024-001…"
          className="flex-1 rounded-md border border-neutral-300 px-3 py-2 text-sm outline-none focus:border-[color:var(--teal)]"
        />
      ...
      <div className="mt-3 flex-1 space-y-1 overflow-y-auto">
        ...
        {!buscando && resultados.length === 0 && busqueda && <p className="text-xs text-neutral-400">Sin resultados.</p>}
        {resultados.map((c) => (
          <button
            key={c.id}
          ...
          >
            ...
            <br />
            <span className="text-neutral-500">{c.gar_id} · {c.expediente} · {c.unidad}</span>
          </button>
        ))}
      </div>
    </div>
