...
import { FileText, Plus, Loader2, MoreVertical, PenLine, Archive, Trash2, LayoutGrid, Inbox, FileCheck2, Archive as ArchiveIcon, Eye, X, RotateCcw, FolderPlus, FolderInput, Layers, KeyRound } from "lucide-react";
import { APODERADO_KEYS } from "@/lib/apoderados";
...
function MenuAcciones({ c, onCambio }: { c: ContratoGenerado; onCambio: () => void }) {
  ...
  // "Elaborar Recepción de Propiedad": manda este cliente/garantía al machote
  // de Acta de Entrega-Recepción de Posesión, jalando lo que ya se conoce
  // (cliente, apoderado, garantía, valor) — el resto (fechas, calendario,
  // ficha) se completa a mano en el nuevo machote.
  const elaborarRecepcion = () => {
    const v = (c.valores ?? {}) as Record<string, unknown>;
    const mapeado: Record<string, unknown> = {
      folioGarantia: v.folioGarantia ?? "",
      nombreGarantia: v.nombreGarantia ?? v.garantiaNueva ?? "",
      domicilioGarantia: v.direccionGarantia ?? v.garantiaNueva ?? "",
      valorOperacion: v.valorOperacion ?? "",
      nombreCliente: v.nombreCliente ?? c.nombre_cliente ?? "",
      telefonoCliente: v.telefonoCliente ?? "",
      correoCliente: v.correoCliente ?? "",
      clienteComparecePorApoderado: v.clienteComparecePorApoderado ?? false,
      nombreApoderadoCliente: v.nombreApoderadoCliente ?? "",
      poderApoderadoCliente: v.poderApoderadoCliente ?? "",
    };
    for (const k of Object.values(APODERADO_KEYS)) if (v[k] != null) mapeado[k] = v[k];
    sessionStorage.setItem("reelaborar_contrato", JSON.stringify({ tipo: "acta_entrega_posesion", valores: mapeado }));
    navigate({ to: "/contratos/editor", search: { tipo: "acta_entrega_posesion" } });
  };
      ...
      {abierto && (
        <>
          ...
          <div className="absolute right-0 z-20 mt-1 w-52 rounded-md border border-border bg-white py-1 shadow-lg">
            ...
            <button onClick={() => { setAbierto(false); elaborarRecepcion(); }} className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-muted">
              <KeyRound className="h-3.5 w-3.5" /> Elaborar Recepción de Propiedad
            </button>
            {c.estado !== "archivado" && (
